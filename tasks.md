...
- [x] **1.1** ModusToolbox → Project Creator → board `KIT_PSC3M5_EVK` → "Hello World" example → Build → Program. *Done when: LED blinks.*
- [x] **1.2** Find COM port (Device Manager → Ports), open serial terminal at 115200 baud. *Done when: text from the board appears.*
...
- [x] **1.4** PC: `python -m venv .venv`, install `tensorflow numpy scipy matplotlib`. *Done when: `import tensorflow` works.*
- [x] **1.5** Download MaFaulDa + CWRU; load one file, plot waveform. *Done when: signal visible.*
